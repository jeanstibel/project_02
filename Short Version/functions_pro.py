...
def get_top_10s(df_data):
    metrics = ['number_of_trades', 'volume', 'diff_oc', 'return', 'vol_change', 'volatility', 'ma_5', 'ma_10']
    top_10s = pd.concat([df_data.nlargest(10, metric)[['crypto_id', metric]].drop_duplicates(subset=['crypto_id']).set_index('crypto_id') for metric in metrics], axis=1)
    return top_10s
